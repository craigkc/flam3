...
static void iter_thread(void *fth)
{
  ...
  for (sub_batch = 0; sub_batch < ficp->batch_size; sub_batch += SBS)
  {
    ...
    sub_batch_size = (sub_batch + SBS > ficp->batch_size) ? (ficp->batch_size - sub_batch) : SBS;

    if (fthp->first_thread && newt != *(ficp->progress_timer))
    {
      double percent = 100.0 *
                       ((((sub_batch / (double)ficp->batch_size) + ficp->temporal_sample_num) / ficp->ntemporal_samples) + ficp->batch_num) / ficp->nbatches;
      int old_mark = 0;
      int ticker;
      ...
      *(ficp->progress_timer) = newt;
      ...
      if (percent > 0)
      {
        eta = (100 - percent) * (*(ficp->progress_timer) - ficp->progress_timer_history[old_mark]) / (percent - ficp->progress_history[old_mark]);
    ...
    for (batch_num = 0; batch_num < nbatches; batch_num++)
    {
      ...
      for (temporal_sample_num = 0; temporal_sample_num < ntemporal_samples; temporal_sample_num++)
      {
        ...
        if (1)
        {
          ...
          switch (field)
          {
          case flam3_field_both:
            shift = 0.0;
            break;
          case flam3_field_even:
            shift = -0.5;
            break;
          case flam3_field_odd:
            shift = 0.5;
            break;
          }
          shift = shift / ppux;
      ...
      k1 = (cp.contrast * cp.brightness *
            PREFILTER_WHITE * 268.0 * batch_filter[batch_num]) /
           256;
