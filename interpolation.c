                         ...
                         flam3_genome *cpi, double *c, double stagger)
{
  ...
  for (i = 0; i < cpi[0].num_xforms; i++)
  {
    ...
    if (result->xform[i].color < 0)
      result->xform[i].color = 0;
    if (result->xform[i].color > 1)
      result->xform[i].color = 1;

    INTERP(xform[i].color_speed);
    if (result->xform[i].color_speed < 0)
      result->xform[i].color_speed = 0;
    if (result->xform[i].color_speed > 1)
      result->xform[i].color_speed = 1;
  ...
  for (i = 0; i < nsrc; i++)
  {
    ...
    for (xf = 0; xf < max_nx + max_fx; xf++)
    {
      ...
      if (dst[i].xform[xf].padding == 1 && !already_aligned)
      {
        ...
        if ((i == 0 && dst[i].interpolation_type == flam3_inttype_log) || (i > 0 && dst[i - 1].interpolation_type == flam3_inttype_log))
        {
