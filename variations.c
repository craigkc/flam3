...
void var35_gaussian(flam3_iter_helper *f, double weight)
{
  ...
  r = weight * (flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) - 2.0);
...
void var36_radial_blur(flam3_iter_helper *f, double weight)
{
  ...
  /* Get pseudo-gaussian */
  rndG = weight * (flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) - 2.0);
...
void var50_supershape(flam3_iter_helper *f, double weight)
{
  ...
  r = weight * ((myrnd * flam3_random_isaac_01(f->rc) + (1.0 - myrnd) * f->precalc_sqrt) - f->xform->super_shape_holes) * pow(t1 + t2, f->xform->super_shape_pneg1_n1) / f->precalc_sqrt;
...
void var67_pre_blur(flam3_iter_helper *f, double weight)
{
  ...
  /* Get pseudo-gaussian */
  double rndG = weight * (flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) + flam3_random_isaac_01(f->rc) - 2.0);
...
void var69_oscope(flam3_iter_helper *f, double weight)
{
  ...
  {
    t = f->xform->oscope_amplitude * exp(-fabs(f->tx) * f->xform->oscope_damping) * cos(tpf * f->tx) + f->xform->oscope_separation;
  }
