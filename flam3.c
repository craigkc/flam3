...
int flam3_colorhist(flam3_genome *cp, int num_batches, randctx *rc, double *hist)
{
  ...
  for (lp = 0; lp < num_batches; lp++)
  {
    ...
    for (plp = 0; plp < 4 * sbs; plp += 4)
    {
      ...
      if (mycolor < 0)
        mycolor = 0;
      if (mycolor > CMAP_SIZE_M1)
        mycolor = CMAP_SIZE_M1;
...
#define APPMOT(x)                                            \
  do                                                         \
  {                                                          \
    addto->x += mot[i].x * motion_funcs(func, freq * blend); \
  } while (0);
  ...
  *ncps = loc_all_ncps;
  ...
  if (*ncps > 1)
  {
    ...
    for (i = 1; i < *ncps; i++)
    {
      ...
      if (flam3_inttype_compat != loc_all_cp[i - 1].interpolation_type && flam3_inttype_older != loc_all_cp[i - 1].interpolation_type)
      {
...
void flam3_mutate(flam3_genome *cp, int mutate_mode, int *ivars, int ivars_n, int sym, double speed, randctx *rc, char *action)
{
  ...
  else if (mutate_mode == MUTATE_POST_XFORMS)
  {
    ...
    for (i = 0; i < cp->num_xforms; i++)
    {
      ...
      if (copy)
      { /* Copy the post from the first xform to the rest of them */
        for (j = 0; j < 3; j++)
        {
...
double flam3_render_memory_required(flam3_frame *spec)
{
  ...
  return (double)cps[0].spatial_oversample * cps[0].spatial_oversample *
